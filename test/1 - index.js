...
describe('ДЗ 5.1 - DOM Events', () => {
  ...
  describe('delegate', () => {
    it('должна добавлять обработчик кликов, который реагирует только на клики по кнопкам', () => {
      ...
      assert(!passed);
      target.children[0].dispatchEvent(
        new CustomEvent(eventName, { bubbles: true }),
      );
      assert(!passed);
      target.children[1].dispatchEvent(
        new CustomEvent(eventName, { bubbles: true }),
      );
      assert(!passed);
      target.children[2].dispatchEvent(
        new CustomEvent(eventName, { bubbles: true }),
      );
      assert(!passed);
      target.children[3].dispatchEvent(
        new CustomEvent(eventName, { bubbles: true }),
      );
      assert(passed);
    });
