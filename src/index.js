...
function collectDOMStat(root) {
  ...
  function anyliseDom(element) {
    for (const child of [...element.childNodes]) {
      if (child.tagName) {
        child.tagName in tags
          ? (tags[child.tagName] = tags[child.tagName] + 1)
          : (tags[child.tagName] = 1);
      }
      if (child.className) {
        child.className.split(' ').forEach(classItem => {
          classItem in classes
            ? (classes[classItem] = classes[classItem] + 1)
            : (classes[classItem] = 1);
        });
      }
